...
public:
	ResidentSpaceObject();
	ResidentSpaceObject(int ID, int numSegments, cSatellite* m_satellite, cJulian* localEpoch, elsetrec* TLEData);
	ResidentSpaceObject(const ResidentSpaceObject& rhs);
	
	virtual ~ResidentSpaceObject();

	ResidentSpaceObject& operator=(const ResidentSpaceObject& rhs);

	virtual void copy(const ResidentSpaceObject& rhs);
	virtual void clear();
