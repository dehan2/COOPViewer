...
#include "Sphere.h"
#include <string>
struct MomentNClosestNeighborRSOsID
{
    int targetRSOID;
    double moment;
    list<int> closestNeighborRSOsID;
};
class OrbitClosestNeighbor
{
public:
    std::list<MomentNClosestNeighborRSOsID> m_momentNClosestNeighborRSOsID;

    OrbitClosestNeighbor();
    void load_orbit_closest_neighbors(const std::string& filePath);
    const std::list<int> find_closest_neighbor_RSOs_ID_in_closest_moment(const double& targetMoment);
};
